...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout,
                            ...
                            QGroupBox, QFrame, QSplitter, QScrollArea, QSizePolicy,
...
class UpdateCheckThread(QThread):
    ...
    def run(self):
        ...
        if latest and url and is_newer(latest, __version__):
...
class UpdateDownloadDialog(QDialog):
    def __init__(self, version, download_url, parent=None):
        ...
        self.label = QLabel(f"버전 {version} 다운로드 중...")
...
class _DownloadThread(QThread):
    ...
    def run(self):
        ...
        path = download_update(self.url, _cb)
        if path:
...
class ModernFileCopier(QMainWindow):
    ...
    def setup_ui(self):
        ...
        content_layout.addWidget(left_panel, 3)  # 더 큰 비중
        
        # 오른쪽 패널
        right_panel = self.setup_right_panel()
        content_layout.addWidget(right_panel, 2)  # 적절한 비중
    ...
    def _create_update_banner(self):
        ...
        banner.setStyleSheet(
            "QFrame { background-color: #d4edda; border: 1px solid #c3e6cb; border-radius: 6px; }"
        )
        row = QHBoxLayout(banner)
        row.setContentsMargins(12, 8, 12, 8)

        self._update_label = QLabel()
        self._update_label.setStyleSheet("color: #155724; font-size: 13px;")
        row.addWidget(self._update_label, 1)

        btn = QPushButton("지금 업데이트")
        btn.setFixedWidth(120)
        btn.setStyleSheet(
            "QPushButton { background-color: #28a745; color: white; border-radius: 4px; padding: 5px 10px; }"
    ...
    def _on_update_available(self, version, url):
        ...
        self._update_label.setText(f"새 버전 {version} 이 있습니다.")
    ...
    def _do_update(self):
        ...
        if not url:
            return
        dlg = UpdateDownloadDialog(self._update_label.text(), url, self)
        if dlg.exec_() == QDialog.Accepted and dlg.new_exe_path:
            apply_update(dlg.new_exe_path)
    ...
    def setup_left_panel(self):
        ...
        layout.addWidget(path_group, 0)  # 고정 크기
        ...
        input_content_layout.addWidget(self.numbers_text, 1)  # 1은 스트레치 팩터
        ...
        layout.addWidget(input_content_group, 2)  # 큰 비중으로 확장 가능
    ...
    def setup_right_panel(self):
        ...
        layout.addWidget(extension_group, 0)  # 고정 크기
        ...
        layout.addWidget(progress_group, 0)  # 고정 크기
        ...
        log_layout.addWidget(self.log_text, 1)  # 1은 스트레치 팩터
        
        layout.addWidget(log_group, 3)  # 큰 비중으로 확장 가능
        ...
        layout.addWidget(creator_group, 0)  # 고정 크기
    ...
    def select_files(self):
        ...
        if files:
            # 선택된 파일들의 파일명만 추출 (확장자 제거)
            file_names = []
            for file_path in files:
                file_name = Path(file_path).stem  # 확장자 제거
                file_names.append(file_name)
            
            # 파일명들을 텍스트로 변환
            file_names_text = '\n'.join(file_names)
            self.numbers_text.setPlainText(file_names_text)
    ...
    def get_input_list(self):
        ...
        for line in lines:
            ...
            # 각 줄에서 쉼표, 슬래시, 공백으로 분리
            # 파일 경로나 파일명이 아닌 경우에만 분리
            if os.path.sep in line or '.' in line:
                # 파일 경로나 파일명인 경우 그대로 추가
                items.append(line)
            else:
                # 숫자나 단순 텍스트인 경우 구분자로 분리
    ...
    def find_files(self, source_dir, input_items):
        ...
        for item in input_items:
            ...
            if os.path.exists(item):
                ...
                # 3. 파일명만으로 찾기 (확장자 무시, 선택한 확장자로 매칭)
                if not found:
                    for file_path in all_files:
                        file_name_without_ext = file_path.stem
                        
                        # 입력 항목과 파일명(확장자 제거)이 일치하는지 확인
                        if file_name_without_ext == item:
                            found_files.append((item, file_path))
                            found = True
                ...
                if not found:
                    for file_path in all_files:
                        ...
                        # 5. 파일명에 포함된 경우 (호환성)
                            ...
                            found_files.append((item, file_path))
                            found = True
                            self.log(f"포함으로 찾음: {item} -> {file_name}")
                            break
